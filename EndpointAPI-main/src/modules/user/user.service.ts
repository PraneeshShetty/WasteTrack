...
export class UserService {
  ...
  async createUser(user: User): Promise<User> {
    // If no roleId is provided, assign the default user role
    if (!user.roleId) {
      const defaultRole = await this.roleModel.findOne({ isDefault: true }).exec();
      if (defaultRole) {
  ...
  async initDefaultUserWithRole(): Promise<void> {
    ...
    user.lastname = 'Admin';
    user.email = 'admin@praneesh.com';
    user.location = 'mangalore';
